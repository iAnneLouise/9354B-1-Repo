...
public class RevisedBobsAndStages{
	...
	void solve() {
		...
		for (int i = 0, tmp = 0; i < n; i++) {
			for (int j = 0; j < n; j++) {
				if (i != j) {
					vecs[tmp] = new Bobby(a[j].x - a[i].x, 
                                                a[j].y - a[i].y);
					vecs[tmp].id1 = i;
					vecs[tmp].id2 = j;
					tmp++;
		...
		for (int start = 0; start < n; start++) {
			...
			for (int i = 0; i < size; i++) {
				for (int j = i + 1; j < size; j++) {
					...
					if (ok) {
						cost[p1][p2] = vectMul(buf[i], 
                                                        buf[j]);
					}
				}
			}
			for (int i = 0; i < size; i++) {
				cost[start][buf[i].id1] = 
                                        cost[buf[i].id1][start] = 0;
			}
